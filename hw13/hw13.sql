...
CREATE TABLE schedule AS
  with recurse_flights(flight, departure_airport, arrival_airport, cost, n) as (
    SELECT departure || ", " || arrival, departure, arrival, price, 1 FROM flights WHERE departure = "SFO" UNION
    SELECT flight || ", " || arrival, departure, arrival, cost + price, n+1 FROM recurse_flights, flights
    WHERE arrival_airport = departure AND n < 2
  )
  SELECT flight, cost FROM recurse_flights WHERE arrival_airport = "PDX" ORDER BY cost;

CREATE TABLE number_of_options AS
  SELECT COUNT(DISTINCT meat) FROM main_course;

CREATE TABLE calories AS
  SELECT COUNT(main.calories + pie.calories) FROM main_course AS main, pies AS pie
  WHERE main.calories + pie.calories < 2500;

CREATE TABLE healthiest_meats AS
  SELECT main.meat, min(main.calories + pie.calories) FROM main_course AS main, pies AS pie
  GROUP BY meat HAVING max(main.calories + pie.calories) < 3000
;
...
CREATE TABLE lowest_prices AS
  SELECT store, name, min(price) FROM inventory, products
  WHERE name = item
  GROUP BY item;

CREATE TABLE shopping_list AS
 with value_tbl(item_name,value) AS (
    SELECT name, min(MSRP / rating) FROM inventory, products
    GROUP BY category
  )
  SELECT item_name, store FROM value_tbl, lowest_prices
