(define (how-many-dots s)
    ; base case - s is empty (nil)
    (if (or (null? s) (number? s))
        0
        ; check if first element is a pair (therefore nested)
        (if (pair? (car s))
            ; if it is nested, check if last element is a number
            (if (number? (cdr s))
                (+ 1 (how-many-dots (car s)))
                (+ (how-many-dots (car s)) (how-many-dots (cdr s)))
            )
            ; if first element is not a list, check that cdr is not a number
            (if (number? (cdr s))
...
(define (derive-sum expr var)
  ; differentiate cadr and caddr then add them together
